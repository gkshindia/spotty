...
class ConfigManager:
        ...
        return {
            "system": {
                ...
                "scaling_cooldown": int(
                    os.environ.get("SCALING_COOLDOWN", "300")
                ),  # 5 minutes
            },
            "aws": {
                "region": aws_region,
                "default_image_id": os.environ.get(
                    "AWS_DEFAULT_IMAGE_ID", "ami-0c55b159cbfafe1f0"
                ),  # Amazon Linux 2
                "instance_types": {
            ...
            try:
                with open(template_file, "r") as f:
                    ...
                    if template_name:
                        ...
                        logger.info(
                            f"Loaded template '{template_name}' from {template_file}"
                        )
                    else:
                        logger.warning(
                            f"Template file {template_file} has no name field, skipping"
                        )
            except Exception as e:
    ...
    def get_appropriate_instance_type(self, workload_template: str) -> str:
        ...
        if not template:
            logger.warning(
                f"Unknown template: {workload_template}, using default CPU instance"
            )
            return (
                self.config.get("aws", {})
                .get("instance_types", {})
        ...
        if requires_gpu:
            gpu_instances = (
                self.config.get("aws", {}).get("instance_types", {}).get("gpu", [])
            )
            if gpu_instances:
                return gpu_instances[0]
            # Fallback if GPU is requested but none are configured
            logger.warning(
                "GPU requested but no GPU instances configured, using CPU instance"
            )

        # Use CPU instance
        cpu_instances = (
            self.config.get("aws", {}).get("instance_types", {}).get("cpu", [])
        )
        if cpu_instances:
