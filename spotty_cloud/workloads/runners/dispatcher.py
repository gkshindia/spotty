...
class WorkloadDispatcher:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.check_interval = config.get("workloads", {}).get(
            "check_interval", 30
        )  # seconds
    ...
    def dispatch_workload(self, workload_id: str, instance_id: str) -> bool:
        ...
        if workload["status"] not in ["pending", "failed"]:
            logger.warning(
                f"Workload {workload_id} is in status {workload['status']}, not dispatchable"
            )
            return False
        ...
        try:
            ...
            logger.error(
                f"Error dispatching workload {workload_id}: {str(e)}", exc_info=True
            )
    ...
    def register_completion_callback(self, workload_id: str, callback) -> bool:
        ...
        if workload_id not in self.workloads:
            logger.warning(
                f"Cannot register callback: Workload {workload_id} not found"
            )
            return False
    ...
    def handle_instance_failure(self, instance_id: str) -> List[str]:
        ...
        logger.info(
            f"Handling failure of instance {instance_id} with {len(affected_workloads)} affected workloads"
        )
        ...
        for workload_id in affected_workloads:
            if workload_id in self.workloads:
                ...
                if workload.get("status") == "running":
                    # Mark as failed
                    workload["status"] = "failed"
                    workload["logs"].append(
                        f"Failed due to instance {instance_id} failure"
                    )
                    ...
                    if workload["retries"] < self.max_retries:
                        logger.info(
                            f"Workload {workload_id} will be retried (attempt {workload['retries'] + 1}/{self.max_retries})"
                        )
                        workload["status"] = "pending"  # Mark for retry
                    else:
                        logger.warning(
                            f"Workload {workload_id} has failed permanently after {workload['retries']} retries"
                        )
                        workload["status"] = "failed"
                        ...
                        if workload_id in self.completion_callbacks:
                            try:
                                self.completion_callbacks[workload_id](
                                    workload_id, "failed"
                                )
                                del self.completion_callbacks[workload_id]
                            except Exception as e:
                                logger.error(
                                    f"Error in completion callback for workload {workload_id}: {str(e)}"
                                )
    ...
    def _check_workload_timeouts(self):
        ...
        for workload_id, workload in list(self.workloads.items()):
            ...
            if start_time and (current_time - start_time) > timeout:
                logger.warning(
                    f"Workload {workload_id} has timed out after {timeout} seconds"
                )
    ...
    def _simulate_workload_execution(self, workload_id: str, instance_id: str):
        ...
        logger.info(
            f"Simulating execution of {workload_type} workload {workload_id} for {execution_time} seconds"
        )
        ...
        for progress in [25, 50, 75]:
            ...
            if (
                workload_id not in self.workloads
                or self.workloads[workload_id].get("status") != "running"
            ):
                return
