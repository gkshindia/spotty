...
def parse_args():
    ...
    parser.add_argument('--video', type=str, choices=list(SAMPLE_VIDEOS.keys()), 
                        default='big_buck_bunny_720p',
                        help='Sample video to download')
    parser.add_argument('--output', type=str, default='./input/sample',
...
def main():
    ...
    output_path = os.path.join(args.output, f"{args.video}.{video['url'].split('.')[-1]}")
