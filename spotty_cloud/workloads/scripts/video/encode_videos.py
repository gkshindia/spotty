...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("encoding.log"), logging.StreamHandler()],
)
...
def download_sample_video(sample_name, output_dir):
    ...
    # Download the file
    print(
        f"Downloading sample video: {sample['description']} (~{sample['size_mb']} MB)"
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--use_gpu", type=bool, default=False, help="Use GPU acceleration for encoding"
    )
    parser.add_argument(
        "--workers", type=int, default=4, help="Number of parallel encoding workers"
    )
    parser.add_argument(
        "--crf",
        type=int,
    ...
    parser.add_argument(
        "--url", type=str, default=None, help="URL to download a custom video"
    )
...
def encode_video(
    input_path, output_path, resolution_key, resolution, use_gpu=False, crf=23
):
    try:
        ...
        cmd.extend(
            [
                "-c:v",
                "libx264" if not use_gpu else "h264_nvenc",
                "-preset",
                "slow" if not use_gpu else "p4",
                "-crf",
                str(crf) if not use_gpu else "0",  # CRF for CPU, not used with GPU
                "-b:v",
                resolution["bitrate"],  # Target bitrate
                "-maxrate",
                resolution["bitrate"],  # Maximum bitrate
                "-bufsize",
                resolution["bitrate"],  # Buffer size
                "-vf",
        ...
        if result.returncode == 0:
            ...
            logger.info(
                f"Successfully encoded {os.path.basename(input_path)} to {resolution_key} in {duration:.2f} seconds"
            )
            return True
        else:
            logger.error(
                f"Error encoding {input_path} to {resolution_key}: {result.stderr}"
            )
            return False
    except Exception as e:
...
def process_video(video_path, output_dir, formats, use_gpu, crf):
    try:
        ...
        logger.info(
            f"Original video: {video_info.get('width')}x{video_info.get('height')}, codec: {video_info.get('codec_name')}"
        )
        ...
        for format_key in formats:
            ...
            output_file = os.path.join(
                output_dir, video_name, f"{video_name}_{format_key}.mp4"
            )
            ...
            if (
                int(video_info.get("width", 0)) < resolution["width"]
                or int(video_info.get("height", 0)) < resolution["height"]
            ):
                logger.warning(
                    f"Source resolution ({video_info.get('width')}x{video_info.get('height')}) is lower than requested {format_key}, skipping"
                )
                continue

            # Encode to the target resolution
            if not encode_video(
                video_path, output_file, format_key, resolution, use_gpu, crf
            ):
                success = False
...
def print_encoding_guidance():
    ...
    print(
        "  - Use when: Quality is priority over speed, need specific encoding features"
    )
    ...
    print(
        "  - Best for: Content distribution, large batch encodes, time-sensitive work"
    )
...
def main():
    ...
    if args.url:
        ...
        if download_file(args.url, output_path):
            ...
            logger.error(
                "Failed to download video from URL. Continuing with existing files."
            )

    # Parse requested formats
    formats = [
        f.strip() for f in args.formats.split(",") if f.strip() in RESOLUTION_PRESETS
    ]
    if not formats:
        logger.error(
            f"No valid formats specified. Available formats: {', '.join(RESOLUTION_PRESETS.keys())}"
        )
        return
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for video_path in video_files:
            future = executor.submit(
                process_video, video_path, args.output, formats, args.use_gpu, args.crf
            )
    ...
    logger.info(
        f"Encoding complete. Successfully processed {successful}/{len(video_files)} videos."
    )
