...
class EdgeProcessor:
    ...
    def process_data(self):
        ...
        if self.sensor_type == "camera":
            data["analysis_result"] = {
                ...
                "classifications": ["person", "car", "tree"] if data.get("objects_detected", 0) > 0 else []
            }
        ...
        # Save to output file
        output_file = os.path.join(self.output_dir, f"{self.sensor_type}_{int(time.time())}.json")
        with open(output_file, 'w') as f:
            json.dump(data, f, indent=2)
            
        logger.info(f"Processed data point {self.data_points} from {self.sensor_type} sensor")
    ...
    def save_checkpoint(self):
        ...
        checkpoint_file = os.path.join("/data/checkpoints", f"{self.sensor_type}_checkpoint.json")
...
def main():
    ...
    parser.add_argument("--sensor", type=str, default="camera", 
    ...
    parser.add_argument("--duration", type=int, default=60,
                        help="Total simulation duration in minutes")
    ...
    logger.info(f"Mode: {args.mode}, Interval: {args.interval}s, Duration: {args.duration} minutes")
    ...
    try:
        ...
        logger.info(f"Simulation complete. Processed {processor.data_points} data points.")
