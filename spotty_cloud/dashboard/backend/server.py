    ...
    try:
        if GLOBAL_ORCHESTRATOR and hasattr(GLOBAL_ORCHESTRATOR, 'workload_dispatcher'):
            ...
            for workload_id, details in workloads.items():
                result[workload_id] = GLOBAL_ORCHESTRATOR.get_workload_status(workload_id)
...
def start_dashboard_server(port: int = 8080, host: str = '0.0.0.0', 
                           debug: bool = False, config: Optional[Dict[str, Any]] = None,
                           orchestrator = None) -> threading.Thread:
    """
