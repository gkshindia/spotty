...
from encode_videos import RESOLUTION_PRESETS, SAMPLE_VIDEOS, download_sample_video, download_file
...
def get_resource_usage():
    try:
        ...
        cpu_usage = np.random.randint(10, 90) if st.session_state.active_jobs > 0 else np.random.randint(0, 20)
        
        # Memory usage (simplified)
        memory_usage = np.random.randint(20, 80) if st.session_state.active_jobs > 0 else np.random.randint(10, 30)
        
        # GPU usage (if available, simplified)
        gpu_usage = np.random.randint(30, 100) if st.session_state.active_jobs > 0 and st.session_state.use_gpu else 0
...
def encoding_worker(job_id, input_path, output_dir, formats, use_gpu, crf, result_queue):
    try:
        ...
        for i, fmt in enumerate(formats_list):
            # Update progress
            progress = {"job_id": job_id, "status": "running", "progress": (i / total_formats) * 100}
        ...
        result_queue.put({"type": "log", "message": f"Job {job_id}: All formats completed for {os.path.basename(input_path)}"})    
    except Exception as e:
...
def create_encoding_job(input_path, output_dir, formats, use_gpu, crf):
    ...
    st.session_state.logs.append(f"Job {job_id}: Created new encoding job for {os.path.basename(input_path)}")
...
def process_job_updates():
    if 'job_queues' in st.session_state and st.session_state.job_queues:
                    ...
                    if "type" in update and update["type"] == "log":
                        ...
                        for job in st.session_state.jobs:
                            if job["id"] == job_id:
                                ...
                                if update["status"] == "completed" or update["status"] == "failed":
        ...
        # Count active jobs
        st.session_state.active_jobs = sum(1 for job in st.session_state.jobs if job["status"] == "running")
...
def update_resource_history():
    ...
    if len(st.session_state.resource_history['timestamps']) > max_history:
        st.session_state.resource_history['timestamps'] = st.session_state.resource_history['timestamps'][-max_history:]
        st.session_state.resource_history['cpu'] = st.session_state.resource_history['cpu'][-max_history:]
        st.session_state.resource_history['memory'] = st.session_state.resource_history['memory'][-max_history:]
        st.session_state.resource_history['gpu'] = st.session_state.resource_history['gpu'][-max_history:]
...
with st.sidebar:
    ...
    # GPU/CPU selector
    st.session_state.use_gpu = st.checkbox("Use GPU for encoding", value=st.session_state.use_gpu)
    
    st.subheader("Video Source")
    source_type = st.radio(
        "Select video source",
        options=["Sample Videos", "URL", "Local Files"],
        index=0
    )
    ...
    if source_type == "Sample Videos":
        ...
        try:
            ...
            local_files = [os.path.basename(f) for f in local_files] or ["No video files found"]
        except Exception as e:
    ...
    if st.button("Start Encoding"):
        if not selected_formats:
            ...
            if source_type == "Sample Videos":
                ...
                st.session_state.logs.append(f"Downloading sample video: {selected_sample}")
                input_path = download_sample_video(selected_sample, input_base_dir) or ""
            ...
            elif source_type == "Local Files" and selected_file and selected_file != "No video files found":
...
tab1, tab2, tab3, tab4 = st.tabs(["Active Jobs", "Job History", "Resource Monitor", "Logs"])
...
with tab1:
    ...
    if not st.session_state.jobs or all(job["status"] in ["completed", "failed"] for job in st.session_state.jobs):
        st.info("No active encoding jobs. Use the sidebar to start a new job.")
    else:
        active_jobs = [job for job in st.session_state.jobs if job["status"] not in ["completed", "failed"]]
        ...
        for job in active_jobs:
            ...
            with col2:
                st.write(f"Job {job['id']}: {job['status'].capitalize()} - {job['progress']:.1f}%")
...
with tab3:
    ...
    if len(st.session_state.resource_history['timestamps']) > 1:
        ...
        timestamps = [ts.strftime("%H:%M:%S") for ts in st.session_state.resource_history['timestamps']]
        
        # Plot CPU and memory
        ax.plot(timestamps, st.session_state.resource_history['cpu'], label="CPU %", marker='o', linestyle='-', alpha=0.7)
        ax.plot(timestamps, st.session_state.resource_history['memory'], label="Memory %", marker='s', linestyle='-', alpha=0.7)
        
        # Plot GPU if used
        if st.session_state.use_gpu:
            ax.plot(timestamps, st.session_state.resource_history['gpu'], label="GPU %", marker='^', linestyle='-', alpha=0.7)
...
try:
    ...
    st.write("Workflow visualization requires graphviz. Install with: pip install graphviz")
