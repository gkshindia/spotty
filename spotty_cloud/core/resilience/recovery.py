...
class RecoveryManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.checkpoint_path = config.get('resilience', {}).get('checkpoint_path', '/data/checkpoints')
    ...
    def start(self):
        ...
        if self.backup_enabled:
            ...
            logger.info(f"Backup thread started with interval {self.backup_interval} seconds")
    ...
    def handle_instance_failure(self, instance_id: str, affected_workloads: List[str], reason: str) -> str:
        """
        ...
        logger.info(f"Initiating recovery {recovery_id} for instance {instance_id} with {len(affected_workloads)} affected workloads")
        ...
        threading.Thread(
            ...
            args=(recovery_id, recovery_state)
        ).start()
    ...
    def backup_instance_data(self, instance_id: str, force: bool = False) -> bool:
        ...
        if not force and (now - last_backup) < self.backup_interval / 2:
            logger.info(f"Skipping backup for {instance_id}: Recent backup exists ({int(now - last_backup)} seconds ago)")
            return False
        ...
        try:
            ...
            logger.error(f"Error during manual backup of instance {instance_id}: {str(e)}", exc_info=True)
            return False
    ...
    def _execute_recovery(self, recovery_id: str, recovery_state: Dict[str, Any]):
        ...
        try:
            ...
            if not backup_info:
                self._update_recovery(recovery_id, 'failed', "No backup found for recovery")
                return
            ...
            self._update_recovery(recovery_id, 'preparing_restore', "Preparing restore environment")
            restore_path = self._prepare_restore_location(instance_id)
            
            self._update_recovery(recovery_id, 'restoring', "Restoring data from backup")
            ...
            if not restore_success:
                self._update_recovery(recovery_id, 'failed', "Failed to restore from backup")
                return
                
            self._update_recovery(recovery_id, 'restored', "Data successfully restored")
            
            self._update_recovery(recovery_id, 'preparing_workloads', "Preparing workloads for reassignment")
            workload_paths = {}
            ...
            logger.error(f"Error during recovery {recovery_id}: {str(e)}", exc_info=True)
            ...
            try:
                ...
                for instance_id in active_instances:
                    ...
                    if (now - last_backup) >= self.backup_interval:
                        try:
                            logger.info(f"Auto-backup triggered for instance {instance_id}")
                            success = self._backup_instance(instance_id)
                            ...
                            logger.error(f"Error during auto-backup of instance {instance_id}: {str(e)}")
