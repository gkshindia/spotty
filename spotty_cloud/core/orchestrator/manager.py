...
class OrchestratorManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.instance_manager = SpotInstanceManager(config, aws_credentials=self.aws_credentials)
        self.recovery_manager = RecoveryManager(config)
        self.instance_monitor = InstanceMonitor(config, aws_credentials=self.aws_credentials)
    ...
    def _validate_aws_credentials(self):
        ...
        if not is_valid:
            ...
            logger.warning("Some features may not work correctly without valid AWS credentials")
        else:
    ...
    def start(self):
        ...
        logger.info(f"Orchestrator started, targeting {self.instance_pool_size} instances")
    ...
    def get_system_status(self):
        ...
        return {
            ...
            'running_workloads': self.workload_dispatcher.get_count_by_status('running'),
            'queued_workloads': self.workload_queue.qsize(),
            'health': health_status,
            'aws_credentials': aws_valid,
            'aws_region': self.aws_credentials.region,
            'cost_data': self.cost_tracker.get_summary() if hasattr(self, 'cost_tracker') else None
        }
    ...
    def _check_instance_health(self):
        ...
        for instance_id, instance_details in list(self.active_instances.items()):
            ...
            if health_status.get('status') != 'healthy':
                logger.warning(f"Instance {instance_id} is unhealthy: {health_status.get('reason', 'unknown')}")
                
                self._handle_instance_failure(instance_id, health_status.get('reason', 'unknown'))
    ...
    def _handle_instance_failure(self, instance_id: str, reason: str):
        ...
        self.recovery_manager.handle_instance_failure(instance_id, affected_workloads, reason)
    ...
    def _update_instance_status(self):
        ...
        for request_id, request_details in list(self.pending_instances.items()):
            ...
            if status.get('status') == 'fulfilled':
                ...
                logger.info(f"Instance request {request_id} fulfilled with instance ID {instance_id}")
                ...
                logger.warning(f"Instance request {request_id} failed: {status.get('reason', 'unknown')}")
                del self.pending_instances[request_id]
    ...
    def _scale_instances(self):
        ...
        target_instances = min(self.max_instances, max(self.min_instances, 
                                                      self.instance_pool_size,
                                                      queue_size // 2))
    ...
    def _distribute_workloads(self):
        ...
        """
        available_instances = [
            instance_id for instance_id, details in self.active_instances.items()
            if details.get('status') == 'ready' and len(self.instance_workloads.get(instance_id, [])) < 3
        ]
        ...
        while available_instances and not self.workload_queue.empty():
            try:
                ...
                logger.info(f"Assigning workload {workload_id} to instance {instance_id}")
                success = self.workload_dispatcher.dispatch_workload(workload_id, instance_id)
                
                if success:
                    self.instance_workloads[instance_id].append(workload_id)
                    self.active_instances[instance_id]['workloads'].append(workload_id)
                else:
                    logger.warning(f"Failed to dispatch workload {workload_id}, requeuing")
                    self.workload_queue.put(workload_id)
