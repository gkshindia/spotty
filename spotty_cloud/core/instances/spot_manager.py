...
class SpotInstanceManager:
    ...
    def __init__(self, config: Dict[str, Any], aws_credentials: AWSCredentialManager = None):
        ...
        self.max_price_multiplier = aws_config.get('spot', {}).get('max_price_multiplier', 1.1)
        self.instance_types = aws_config.get('spot', {}).get('instance_types', {
    ...
    def request_instance(self, instance_type: str = None, workload_type: str = None) -> Optional[str]:
        """
        ...
        if not instance_type and workload_type:
            instance_types = self.instance_types.get(workload_type, self.instance_types['cpu'])
        ...
        if workload_type == 'gpu' and not is_gpu_instance:
            logger.warning(f"Requested GPU workload but instance type {instance_type} may not have GPU. Switching to g4dn.xlarge")
            instance_type = 'g4dn.xlarge'
        elif workload_type == 'edge' and not is_edge_instance:
            logger.warning(f"Requested edge workload but {instance_type} is not optimized for edge. Switching to t3.medium")
        ...
        try:
            ...
            logger.info(f"Requesting {instance_type} instance with max price ${max_price:.4f}/hour")
            ...
            logger.info(f"Simulating Spotty API call to launch {instance_type} instance")
    ...
    def check_instance_request(self, request_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if request_age > 60 and request_details.get('status') == 'pending':
                ...
                logger.info(f"Request {request_id} fulfilled with instance {instance_id}")
            ...
            logger.error(f"Error checking spot request {request_id}: {str(e)}", exc_info=True)
            return {
                'status': 'error',
                'reason': str(e)
            }
    ...
    def terminate_instance(self, instance_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Simulating Spotty API call to terminate instance {spotty_name} ({instance_id})")
            ...
            logger.error(f"Error terminating instance {instance_id}: {str(e)}", exc_info=True)
            return False
    ...
    def list_active_instances(self) -> List[Dict[str, Any]]:
        ...
        """
        return [
            details for instance_id, details in self.instances.items()
