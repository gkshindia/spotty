...
class InstanceMonitor:
    ...
    def __init__(
        ...
        self.alert_thresholds = monitoring_config.get(
            "thresholds", {"cpu_high": 90, "memory_high": 85, "disk_high": 90}
        )
        ...
        if not self.session:
            logger.error(
                "Failed to initialize AWS session. Instance monitoring will be limited."
            )
    ...
    def start(self):
        ...
        logger.info(
            f"Instance monitoring thread started with interval {self.monitoring_interval} seconds"
        )
    ...
    def register_instance(self, instance_id: str, instance_type: str) -> bool:
        ...
        if instance_id in self.instances:
            logger.warning(
                f"Instance {instance_id} is already registered for monitoring"
            )
            return False
    ...
    def _check_spot_termination_notices(self):
        ...
        for instance_id in list(self.instances.keys()):
            try:
                ...
                if self.config.get("simulation", {}).get(
                    "simulate_terminations", False
                ):
                    import random

                    if (
                        instance_id not in self.spot_termination_notices
                        ...
                        logger.warning(
                            f"Simulated termination notice for instance {instance_id} at {time.ctime(termination_time)}"
                        )

            except Exception as e:
                logger.error(
                    f"Error checking termination notice for instance {instance_id}: {str(e)}"
                )
    ...
    def _check_instance_health(self):
        ...
        for instance_id in list(self.instances.keys()):
            try:
                ...
                try:
                    ...
                    if response["InstanceStatuses"]:
                        ...
                        if (
                            ...
                            instance_details["message"] = (
                                f"Instance state: {instance_state}, status: {instance_status}, system: {system_status}"
                            )
                ...
                logger.error(
                    f"Error checking health for instance {instance_id}: {str(e)}"
                )

                # Update with error status
                if instance_id in self.instances:
                    self.instances[instance_id]["health"] = "error"
                    self.instances[instance_id]["message"] = (
    ...
    def _update_instance_metrics(self):
        ...
        for instance_id in list(self.instances.keys()):
            try:
                ...
                alerts = []
                if metrics["cpu_utilization"] > self.alert_thresholds.get(
                    "cpu_high", 90
                ):
                    alerts.append(
                ...
                if metrics["memory_utilization"] > self.alert_thresholds.get(
                    "memory_high", 85
                ):
                    alerts.append(
                ...
                if metrics["disk_utilization"] > self.alert_thresholds.get(
                    "disk_high", 90
                ):
                    alerts.append(
                        f"High disk utilization: {metrics['disk_utilization']:.1f}%"
                    )
                ...
                if alerts:
                    logger.warning(
                        f"Instance {instance_id} has {len(alerts)} alerts: {', '.join(alerts)}"
                    )

            except Exception as e:
                logger.error(
                    f"Error updating metrics for instance {instance_id}: {str(e)}"
                )
