        ...
        try:
            ...
            if os.environ.get("AWS_ACCESS_KEY_ID") and os.environ.get(
                ...
                if os.environ.get("AWS_SESSION_TOKEN"):
                    ...
                    session_kwargs["aws_session_token"] = os.environ.get(
                        "AWS_SESSION_TOKEN"
                    )
            ...
            # Try to get credentials from AWS CLI config or EC2 instance role
                ...
                self.session = boto3.Session(region_name=self.region)
                logger.info(
                    "Initialized AWS session from AWS CLI config or instance role"
                )
                return self._validate_credentials()
    ...
    def get_instance_identity() -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "requests module not available, cannot check EC2 instance metadata"
                )
                return {}
