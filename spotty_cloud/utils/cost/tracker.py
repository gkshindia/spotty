...
class CostTracker:
    ...
    def __init__(
        ...
        data_dir = config.get("system", {}).get(
            "data_dir", os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        )
        self.cost_data_file = os.path.join(
            data_dir, cost_config.get("data_file", "cost_data.json")
        )
        ...
        if not self.session:
            logger.error(
                "Failed to initialize AWS session. Cost tracking features will be limited."
            )
        ...
        try:
            ...
            self.pricing_client = self.session.client(
                "pricing", region_name=pricing_region
            )
    ...
    def start(self):
        ...
        logger.info(
            f"Cost tracking thread started with interval {self.tracking_interval} seconds"
        )
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        total_spot_cost = sum(
            details.get("total_cost", 0) for details in self.instance_costs.values()
        )
        total_ondemand_cost = sum(
            details.get("equivalent_ondemand", 0)
            for details in self.instance_costs.values()
        )
        total_savings = (
            total_ondemand_cost - total_spot_cost if total_ondemand_cost > 0 else 0
        )
        savings_percent = (
            (total_savings / total_ondemand_cost * 100)
    ...
    def register_instance(
        self, instance_id: str, instance_type: str, start_time: float
    ) -> bool:
        """
        ...
        if instance_id in self.instance_costs:
            logger.warning(
                f"Instance {instance_id} is already registered for cost tracking"
            )
            return False
    ...
    def update_instance_price(self, instance_id: str, new_spot_price: float) -> bool:
        ...
        if instance_id not in self.instance_costs:
            logger.warning(
                f"Instance {instance_id} is not registered for cost tracking"
            )
            return False
        ...
        logger.info(
            f"Updated spot price for instance {instance_id} to ${new_spot_price:.4f}/hour"
        )
        return True

    def deregister_instance(
        self, instance_id: str, end_time: Optional[float] = None
    ) -> bool:
        """
        ...
        logger.info(
            f"Final cost: ${spot_cost:.2f} (vs. On-demand: ${ondemand_cost:.2f}, Savings: ${savings:.2f})"
        )
    ...
    def _update_historical_data(self):
        ...
        active_instances = len(
            [
                d
                for d in self.instance_costs.values()
                if "status" not in d or d["status"] != "terminated"
            ]
        )
        ...
        if len(self.historical_data["timestamps"]) > max_entries:
            self.historical_data["timestamps"] = self.historical_data["timestamps"][
                -max_entries:
            ]
            self.historical_data["spot_cost"] = self.historical_data["spot_cost"][
                -max_entries:
            ]
            self.historical_data["ondemand_cost"] = self.historical_data[
                "ondemand_cost"
            ][-max_entries:]
            self.historical_data["savings"] = self.historical_data["savings"][
                -max_entries:
            ]
            self.historical_data["instances"] = self.historical_data["instances"][
                -max_entries:
            ]
    ...
    def _get_spot_price(self, instance_type: str) -> float:
        ...
        if not self.session or not self.aws_credentials.credentials_valid:
            logger.warning(
                f"Using default spot price for {instance_type} due to invalid AWS credentials"
            )
    ...
    def _get_ondemand_price(self, instance_type: str) -> float:
        ...
        try:
            ...
            if (
                cache_entry and (time.time() - cache_entry.get("timestamp", 0)) < 86400
            ):  # Cache for 1 day
            ...
            if "PriceList" in response and response["PriceList"]:
                ...
                price = float(
                    price_data.get("terms", {}).get("OnDemand", {}).get("price", 0)
                )
            ...
            logger.warning(
                f"Error getting on-demand price for {instance_type}: {str(e)}"
            )
            return self._get_default_ondemand_price(instance_type)
    ...
    def _load_cost_data(self):
        ...
        try:
            with open(self.cost_data_file, "r") as f:
                ...
                logger.info(
                    f"Loaded cost data for {len(self.instance_costs)} instances"
                )
